...
public:
    ValAndImmInstr(Opcode op, InstrType type, Instruction *val, T imm, bool setId = true)
        : Instruction(op, type, setId), value_(val), imm_(imm) {}

    inline std::string toString() const override {
        return Instruction::toString() + " @" + std::to_string(instrId_) + " @" + std::to_string(value_->GetId()) + " " +
               std::to_string(imm_);
    }
};
...
public:
    JumpInstr(Opcode op, BB *trueJump, Instruction *pred = nullptr, BB *falseJump = nullptr, bool setId = true)
        : Instruction(op, InstrType::VOID, setId), trueOut_(trueJump), falseOut_(falseJump), pred_(pred) {
        assert(trueOut_);
